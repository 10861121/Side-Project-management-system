        ...
        <div class="container-fluid">
            <div class="row">
                ...
                <div class="col-lg-10 mb-4">
                    ...
                    <div class="col-lg-2 mb-4 mt-2"><a href="{{action('HomeController@HomeAdd')}}" class="btn btn-primary">+ 新增</a></div>
                    <table id="table_id" class="display">
                        ...
                        <tbody>
                            @if(isset($DBcarousel))
                                @foreach($DBcarousel as $carousel)
                                    <tr>
                                        <td><img src="{{url('image/carousel/',$carousel->ca_file)}}" style="width:50px;"></td>
                                        <td>{{$carousel->ca_title}}</td>
                                        <td>{{$carousel->ca_order}}</td>
                                        <td>{{$carousel->ca_switch}}</td>
                                        <td><a href="{{action('HomeController@HomeEdit',$carousel->ca_id)}}" class="btn btn-warning">+ 修改</a></td>
                                        <td><a href="{{action('HomeController@HomeDeleteAction',$carousel->ca_id)}}" class="btn btn-danger">+ 刪除</a></td>
                                    </tr>
                                @endforeach
                            @endif
                        </tbody>
