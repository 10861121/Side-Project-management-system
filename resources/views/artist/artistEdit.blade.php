        ...
        <div class="container-fluid">
            <div class="row">
                ...
                <div class="col-lg-10 mb-4">
                    ...
                    @endif
                    <form action="{{action('ArtistController@ArtistEditAction')}}" method="post" enctype="multipart/form-data">
                        @csrf
                         <input type="hidden" class="form-control" name="id" aria-describedby="emailHelp" placeholder="title" value="{{$DBartist->ARTIST_ID}}">
                        <div class="form-group">
                            <label>名字</label>
                            <input type="text" class="form-control" name="name" id="title" aria-describedby="emailHelp" placeholder="t1" value="{{$DBartist->ARTIST_NAME}}">
                        </div>
                        <div class="form-group">
                            <label>英文名字</label>
                            <input type="text" class="form-control" name="ename" id="title" aria-describedby="emailHelp" placeholder="title" value="{{$DBartist->ARTIST_ENNAME}}">
                        </div>
                        <div class="form-group">
                            <label>內容介紹</label>
                            <input type="text" class="form-control" name="content" id="title" aria-describedby="emailHelp" placeholder="title" value="{{$DBartist->ARTIST_CONTENT}}">
                        </div>
                        <div class="form-group">
                            <label>圖片上傳</label>
                            <input type="file" class="form-control" name="image" id="image" onchange="loadFile(event)">
                            <label>目前影像：</label><img style="width:20%;height: 20%;" src="{{url('image/artist/',$DBartist->ARTIST_IMGURL)}}" />
                            <label>更換影像：</label><img id="output" style="width:20%;height: 20%;" src="{{$DBartist->ARTIST_IMGURL}}" />
                        </div>
