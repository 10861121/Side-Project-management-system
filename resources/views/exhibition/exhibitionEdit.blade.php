        ...
        <div class="container-fluid">
            <div class="row">
                ...
                <div class="col-lg-10 mb-4">
                    ...
                    @endif
                    <form action="{{action('ExhibitionConhtroller@ExhibitionEditAction')}}" method="post" enctype="multipart/form-data">
                        @csrf
                         <input type="hidden" class="form-control" name="id" aria-describedby="emailHelp" placeholder="title" value="{{$DBexhibition->EXHIBITION_ID}}">
                        <div class="form-group">
                            <label>標題</label>
                            <input type="text" class="form-control" name="title" id="title" aria-describedby="emailHelp" placeholder="t1" value="{{$DBexhibition->EXHIBITION_TITLE}}">
                        </div>
                        <div class="form-group">
                            <label>展覽時間</label>
                            <input type="text" class="form-control" name="t1" id="title" aria-describedby="emailHelp" placeholder="title" value="{{$DBexhibition->EXHIBITION_T1}}">
                        </div>
                        <div class="form-group">
                            <label>年份</label>
                            <select class="form-select" aria-label="Default select example" name="year">
                                @if(isset($DBexhibition_year))
                                    @foreach($DBexhibition_year as $year)
                                        <option <?php if ($year->EXHIBITION_YEAR_ID == $DBexhibition->EXHIBITION_YEAR_ID) echo "selected"; ?>  value="{{$year->EXHIBITION_YEAR_ID}}">{{$year->EXHIBITION_YEAR}}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <label>目前影像：</label><img style="width:20%;height: 20%;" src="{{url('image/exhibition/',$DBexhibition->EXHIBITION_IMGURL)}}" />
                            <label>更換影像：</label><img id="output" style="width:20%;height: 20%;" src="{{$DBexhibition->EXHIBITION_IMGURL}}" />
                        </div>
