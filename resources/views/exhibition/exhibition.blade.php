            ...
            <div class="row">
                ...
                <div class="col-lg-10 mb-4">
                    <div class="row align-items-center">
                        <div class="col-lg-12 mb-12 mt-2"><h3>展覽管理</h5></div>
                    </div>
                    <div class="col-lg-2 mb-4 mt-2"><a href="{{action('ExhibitionConhtroller@ExhibitionAdd')}}" class="btn btn-primary">+ 新增</a></div>
                        ...
                        <tbody>
                            @if(isset($DBexhibition))
                                @foreach($DBexhibition as $exhibition)
                                    <tr>
                                        <td><img src="{{url('image/exhibition/',$exhibition->EXHIBITION_IMGURL)}}" style="width:50px;"></td>
                                        <td>{{$exhibition->EXHIBITION_TITLE}}</td>
                                        <td>{{$exhibition->EXHIBITION_YEAR}}</td>
                                        <td><a href="{{action('ExhibitionConhtroller@ExhibitionEdit',$exhibition->EXHIBITION_ID)}}" class="btn btn-warning">+ 修改</a></td>
                                        <td><a href="{{action('ExhibitionConhtroller@ExhibitionDeleteAction',$exhibition->EXHIBITION_ID)}}" class="btn btn-danger">+ 刪除</a></td>
                                    </tr>
                                @endforeach
                            @endif
                        </tbody>
