            ...
            <div class="row">
                ...
                <div class="col-lg-10 mb-4">
                    <div class="row align-items-center">
                        <div class="col-lg-12 mb-12 mt-2"><h3>關於我們</h5></div>
                    </div>
                    @if($errors->any())
                        <h5 style="color:red">{{$errors->first()}}</h4>
                    @endif
                    <form action="{{action('AboutController@AboutAction')}}" method="post" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" class="form-control" name="id" aria-describedby="emailHelp" placeholder="title" value="{{$DBabout->ABOUT_ID}}">
                        <div class="form-group">
                            <label>內容介紹</label>
                              <textarea class="form-control" name="content" id="exampleFormControlTextarea1" rows="5" >{{$DBabout->ABOUT_CONTENT}}</textarea>
                        </div>
                        <div class="form-group">
                            <label>圖片上傳</label>
                            <input type="file" class="form-control" name="image" id="image" onchange="loadFile(event)">
                            <label>目前影像：</label><img style="width:20%;height: 20%;" src="{{url('image/about/',$DBabout->ABOUT_IMGURL)}}" />
                            <label>更換影像：</label><img id="output" style="width:20%;height: 20%;" src="{{$DBabout->ABOUT_IMGURL}}" />
                        </div>
